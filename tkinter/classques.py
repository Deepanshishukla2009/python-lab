...
class CalmPulseApp:
    ...
    def show_dashboard_content(self):
        ...
        for i, v in enumerate(values):
            x = i * 70 + 40
            canvas.create_line(x, 180, x, 180 - v * 15, width=10, fill="#4a90e2")
        ...
        # LEFT: Emotional
        emo_frame = tk.Frame(bottom_frame, bg="white")
        emo_frame.pack(side="left")

        tk.Label(emo_frame, text="Emotional Breakdown",
        ...
        for m, v, color in moods:
            f = tk.Frame(emo_frame, bg="white")
            ...
            fill = v * 2
            c.create_rectangle(0, 0, fill, 20, fill=color)
            c.create_polygon(fill, 0, fill+10, 10, fill, 20, fill=color)

            tk.Label(f, text=f"{v}%", bg="white").pack(side="left")

        # RIGHT: Stress Circle
        stress_frame = tk.Frame(bottom_frame, bg="white")
        stress_frame.pack(side="right", padx=50)

        tk.Label(stress_frame, text="Stress Level",
                 font=("Arial", 14, "bold"), bg="white").pack()

        circle = tk.Canvas(stress_frame, width=150, height=150,
                           bg="white", highlightthickness=0)
        circle.pack()

        circle.create_oval(10, 10, 140, 140, outline="#ddd", width=10)
        circle.create_arc(10, 10, 140, 140, start=90, extent=-223,
                          fill="#ff4d4d", outline="")
